...
func TestParseSubscriptionUsageSnapshot(t *testing.T) {
	...
	t.Run("keeps both windows and plan", func(t *testing.T) {
		...
		snap, err := parseSubscriptionUsageSnapshot(body, now)
		require.NoError(t, err)
		assert.Equal(t, &SubscriptionUsageSnapshot{
			PlanType:        "plus",
			PrimaryWindow:   &SubscriptionUsageWindow{UsedPercent: 12.5, ResetAt: 1700003600, LimitWindowSeconds: 18000},
			SecondaryWindow: &SubscriptionUsageWindow{UsedPercent: 40, ResetAt: 1700400000, LimitWindowSeconds: 604800},
			UpdatedAt:       now.Unix(),
		}, snap)
	})
	...
	t.Run("single window and limit reached", func(t *testing.T) {
		...
		assert.Equal(t, &SubscriptionUsageWindow{UsedPercent: 100, LimitWindowSeconds: 604800}, snap.PrimaryWindow)
	...
	t.Run("rejects payload without windows", func(t *testing.T) {
		_, err := parseSubscriptionUsageSnapshot([]byte(`{"plan_type":"plus","rate_limit":{}}`), now)
		require.Error(t, err)
	})

	t.Run("rejects malformed json", func(t *testing.T) {
		_, err := parseSubscriptionUsageSnapshot([]byte(`not json`), now)
		require.Error(t, err)
	})
}
