...
| 04 | frontend-config | done | 2026-07-06 |

## 当前
04 完成（spec 合规 5/5）。全部 4 步完成，进入收尾。
04 自修一笔：Test 用例 hasIssueForField 的 path 参数类型标注比 zod4 $ZodIssue.path(PropertyKey[]) 窄，typecheck 报错；修正类型标注（不动断言逻辑），单测 4/4、typecheck 干净。
遗留（可接受）：前端 UI/声明式（drawer 渲染、图标、key prompt、locale 对称）无单测，由 typecheck+构建+人工核对保证；交互验收待 /test-webview。
