...
- 响应存在两种形态，解析时同时兼容：顶层窗口对象（`five_hour` / `seven_day` / `seven_day_opus`，各含 `utilization` 0–100）；`limits` 数组（各项含 `percent` 0–100）。
...
| `parseClaudeOAuthUsageBottleneck` | 从响应 JSON 提取所有窗口使用率取最大值；无任何窗口报错 |
