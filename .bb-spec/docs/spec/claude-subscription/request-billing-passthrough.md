...
description: 上游 /v1/messages,复用标准 Claude 请求转换与 token 计费;无固定订阅倍率;不提供面向用户/管理端的用量展示。
---

# 请求转换、计费与非目标

## 目的
明确 Claude 订阅渠道复用标准 Claude 管道处理请求与计费,并划定「不做用量展示」的边界。
...
- [ ] 系统不暴露该渠道面向用户/管理端的用量查询端点或前端用量弹窗。
